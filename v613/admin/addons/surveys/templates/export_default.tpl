...
<script type="text/javascript">
	...
	jFrame.getInstance('moduleForm').dispatch('export.responses');
	
</script>

	<h2 class="PageTitle">{$lang.Addon_Surveys_exportResponsesTitle}</h2>
	<div class="PageDescription">{$lang.Addon_Surveys_exportResponsesDescription}</div>
	
	{$flashMessage}
	
	{if $forms}
		<form id="form-responses" action="index.php?Page=Addons&Addon=surveys&Action=export&ajax=1" method="post">	
			<h3 class="Heading2">{$lang.Addon_Surveys_exportResponsesSelectForm}</h3>
			<fieldset class="inline">
				<ul>
					<li>
						<label>
							<span class="required">*</span>
							{$lang.Addon_Surveys_exportResponsesFeedbackForm}
						</label>
						<div>
					...
					<li>
						...
						<div style="margin-left: 172px">
							<button class="Field" type="submit">{$lang.Addon_Surveys_exportResponsesDownload}</button>
							<button class="Field cancel" type="button">{$lang.Addon_Surveys_exportResponsesCancel}</button>
						</div>
					</li>
				</ul>
		</form>
		...
		<tr>
			<td class="body">
				<div style='background-color:#FFF1A8; padding:5px 5px 8px 10px; margin-bottom:10px'>
					<img src='images/info.gif' width='18' height='18' align='left' style='padding-right:4px; margin-top:-2px' />{$lang.Addon_Surveys_exportResponsesNoForms}
				</div>
			</td>
		</tr>
