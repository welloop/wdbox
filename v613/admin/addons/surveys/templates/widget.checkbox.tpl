<script type="text/javascript">
	...
	jFrame.getInstance('moduleForm').dispatch('widget.checkbox', {
			context : '#{$randomId}'
		});

</script>

<li id="{$randomId}" class="form-element {type: 'checkbox'}">
	<input type="hidden" name="widget[{$randomId}][id]" value="{$widget.id}" />
	<input type="hidden" name="widget[{$randomId}][type]" value="checkbox" />
	
	<div class="ui-draggable-handle">
		<div class="form-element-title">
			<span class="form-element-required">*</span>
			<span class="title"><input class="edit-in-place example-field strong" type="text" name="widget[{$randomId}][name]" value="{$widget.name}" title="{$lang.Addon_Surveys_WidgetCheckboxDefaultName}" /></span>
	...
	<div class="form-element-content">
		<input class="edit-in-place example-field em light" type="text" name="widget[{$randomId}][description]" value="{$widget.description}" title="{$lang.Addon_Surveys_WidgetCheckboxDefaultDescription}" />
					...
					<tbody class="form-element-option-list">
						{foreach from=$widgetFields key=fieldIndex item=widgetField}
							<tr>
								<td style="vertical-align: top">
									...
									<input id="selected-{$fieldIndex}-{$randomId}" type="checkbox" name="widget[{$randomId}][field][{$fieldIndex}][is_selected]" value="1" {if $widgetField.is_selected}checked="checked"{/if} />
								</td>
						...
						<tr class="other-row" {if !$widgetFieldOther}style="display: none;"{/if} >
							<td style="vertical-align: top">
								...
								<input type="checkbox" name="widget[{$randomId}][field][other][is_selected]" value="1" {if $widgetFieldOther.is_selected}checked="checked"{/if} />
							</td>
							<td>
								<div>
									<input class="edit-in-place example-field" 
										   ...
										   value="{if $widgetFieldOther}{$widgetFieldOther.other_label_text}{else}{$lang.Addon_Surveys_editResponseOtherText}{/if}"
										   title="{$lang.Addon_Surveys_editResponseOtherText}"/>
								<div>
		...
		<div class="form-element-options">
			<ul class="inline">
				<li>
					...
					<label for="is-required-{$randomId}">{$lang.Addon_Surveys_WidgetCheckboxOptionRequiresAnAnswer}</label>
				</li>
				<li>
					<input id="is-random-{$randomId}" type="checkbox" name="widget[{$randomId}][is_random]" value="1" {if $widget.is_random}checked="checked"{/if} />
					<label for="is-random-{$randomId}">{$lang.Addon_Surveys_WidgetCheckboxOptionRandomize}</label>
					<img class="tooltip" src="images/help.gif" alt="{$lang.Addon_Surveys_WidgetCheckboxTooltipTitleRandom}" title="{$lang.Addon_Surveys_WidgetCheckboxTooltipDescriptionRandom}" />
				</li>
				<li>
					<select name="widget[{$randomId}][is_visible]">
						<option value="1" {if $widget.is_visible == '1'}selected="selected"{/if}>{$lang.Addon_Surveys_WidgetCheckboxOptionVisibleToEveryone}</option>
						<option value="0" {if $widget.is_visible == '0'}selected="selected"{/if}>{$lang.Addon_Surveys_WidgetCheckboxOptionVisibleToAdministrators}</option>
					</select>
					<img class="tooltip" src="images/help.gif" alt="{$lang.Addon_Surveys_WidgetCheckboxTooltipTitleVisibility}" title="{$lang.Addon_Surveys_WidgetCheckboxTooltipDescriptionVisibility}" />
				</li>
				<li class="clear"></li>
			</ul>
		</div>
