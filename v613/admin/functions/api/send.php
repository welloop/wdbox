	...
	*/
	...
	function SetupJob($jobid=0, $queueid=0)
	{
		...
		/* Sendaddress random 
			...
			$nameStr = array("yvonne","mignon","letitia","heloise","quintina","xaviera","cherry","adeline","stacey",
			"bernice","zenobia","carman","mildred","frances","violet","phoebe","veronica","valerie",
			"lesley","lindsay","maxine","crystal","phyllis","candice","suzanne","caroline","melissa",
			"natalie","genevieve","corinna","sherry","shirley","rebecca","jennifer","janice","gloria",
			"priscilla","esther","kimberley","christina","marguerite","josephine","jacqueline","christine",
			"olivia","margaret","vanessa","antonia","hermosa","victoria","trista","diamond","wallis","rosalind",
			"jessica","setlla","aileen","jessie","gwendolyn","sophia","modesty","lauren","charis","sabrina","stephanie");

			$secondArray = array("center","notes","emailbox","trust","cloud","member","ebox","deliver","benefy","exam");
			$thirdArray = array("mx","mta","news","cloud","out","dm","smtp","deliver","mail");
			$domainArray = array("mzbox.net","tdurz.com");

			$secondPre = $secondArray[array_rand($secondArray,1)];
			$thirdPre = $thirdArray[array_rand($thirdArray,1)];
			$domainName = $domainArray[array_rand($domainArray,1)];

			$numPre = mt_rand(1,9);
			$dmzuhe = $thirdPre.$numPre.".".$secondPre.".".$domainName;
			$rand_keys = array_rand($nameStr, 1);
			$finalEmail = $nameStr[$rand_keys]."@".$dmzuhe;

			$this->jobdetails['SendFromEmail'] = $finalEmail;
			$this->jobdetails['ReplyToEmail'] = $finalEmail;
			$this->jobdetails['BounceEmail'] = $finalEmail;
